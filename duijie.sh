...
cleanup_old_duijie() {
    # 检查落地机本地是否有旧对接记录
    if [[ ! -f "$LOCAL_INFO" ]] || ! grep -q "对接节点链接\|RELAY_IP=\|RELAY_PORT=" "$LOCAL_INFO" 2>/dev/null; then
        return 0
    fi

    echo ""
    echo -e "${YELLOW}╔══════════════════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}║  检测到旧的对接记录，是否清除后重新对接？           ║${NC}"
    echo -e "${YELLOW}╚══════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "  将清除："
    echo -e "  ${CYAN}[1]${NC} 落地机 $LOCAL_INFO 中的所有对接记录"
    echo -e "  ${CYAN}[2]${NC} 中转机 config.json 中所有 relay-in-*/relay-out-* 规则"
    echo -e "  ${CYAN}[3]${NC} 中转机 nodes.json 节点注册表"
    echo ""
    read -rp "清除旧数据并重新对接？[y/N]: " yn || true
    [[ "${yn,,}" != "y" ]] && return 0

    # ── 清理落地机本地记录 ──
    log_step "清理落地机旧对接记录..."
    # 只保留 luodi.sh 写入的原始信息（第一段，到第一个"对接节点链接"之前）
    local clean_info
    clean_info=$(awk '/^── 对接节点链接|^RELAY_IP=|────.*对接节点链接/{exit} {print}' "$LOCAL_INFO")
    echo "$clean_info" > "$LOCAL_INFO"
    log_info "落地机旧对接记录已清除"

    # ── 清理中转机 ──
    echo ""
    echo -e "${YELLOW}── 需要 SSH 到中转机清理旧配置 ──${NC}"
    echo -e "  跳过此步骤中转机旧规则会残留，建议清理"
    read -rp "现在连接中转机清理？[Y/n]: " yn2 || true
    [[ "${yn2,,}" == "n" ]] && { log_warn "已跳过中转机清理"; return 0; }

    # 复用 setup_ssh 的逻辑单独建立连接
    read -rp "中转机公网 IP: " _CLEAN_RELAY_IP || true
    [[ -z "$_CLEAN_RELAY_IP" ]] && { log_warn "未输入 IP，跳过中转机清理"; return 0; }
    read -rp "SSH 端口 [22]: " _p || true; _CLEAN_PORT="${_p:-22}"
    read -rp "SSH 用户 [root]: " _u || true; _CLEAN_USER="${_u:-root}"
    local _CLEAN_OPTS="-o StrictHostKeyChecking=no -o ConnectTimeout=10 -p $_CLEAN_PORT"

    echo -e "${YELLOW}认证方式：[1]密钥  [3]密码${NC}"
    read -rp "选择 [1]: " _auth || true; _auth="${_auth:-1}"

    local _CLEAN_RELAY_CONFIG="/usr/local/etc/xray-relay/config.json"
    local _CLEAN_RELAY_NODES="/usr/local/etc/xray-relay/nodes.json"

    _run_clean() {
        local _cmd="$1"
        if [[ "$_auth" == "3" ]]; then
            read -rsp "SSH 密码: " _CLEAN_PASS; echo ""
            sshpass -p "$_CLEAN_PASS" ssh -q $_CLEAN_OPTS "${_CLEAN_USER}@${_CLEAN_RELAY_IP}" "$_cmd"
        else
            ssh -q $_CLEAN_OPTS "${_CLEAN_USER}@${_CLEAN_RELAY_IP}" "$_cmd"
        fi
    }

    log_step "清理中转机旧规则..."
    _run_clean "python3 << 'PYEOF'
import json, sys
...
try:
    ...
    cfg = {}
cfg['inbounds']  = [i for i in cfg.get('inbounds',  []) if not i.get('tag','').startswith('relay-in-')]
cfg['outbounds'] = [o for o in cfg.get('outbounds', []) if not o.get('tag','').startswith('relay-out-')]
...
print('[OK] 中转机旧规则已全部清除')
PYEOF
" || log_warn "中转机清理失败，请手动检查"

    _run_clean "systemctl restart xray-relay 2>/dev/null && echo '[OK] xray-relay 已重启'" || true
    log_info "中转机清理完成"
    echo ""
}
