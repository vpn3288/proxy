...
# ║  🔧 v17 相较 v16 新增修复                                        ║
# ║                                                                  ║
# ║  [J1] generate_substore_json bash变量注入风险                    ║
# ║       v16 直接将 ${NODE_LABEL}/${RELAY_IP}/... 嵌入 Python       ║
# ║       heredoc，含引号或反斜杠时 Python 语法崩溃。               ║
# ║       v17：全部参数先 json.dumps 序列化为 params_json，          ║
# ║       Python 侧 json.loads 安全读取，100% 无注入风险。           ║
# ║                                                                  ║
# ║  [J2] 端口分配：死代码 `if p > 65000: p = start_port`           ║
# ║       v16 _tries 计数器已防无限循环，但该 reset 行与计数器逻辑   ║
# ║       互相干扰，在极端情况下浪费 500 次循环。                   ║
# ║       v17：移除 reset 行，超 65535 时直接 sys.exit(1) 报错。     ║
# ║                                                                  ║
# ║  [J3] Failover 激活时个体路由规则成为死代码                      ║
# ║       v16 写入新节点时先插入单节点路由规则，2+节点时再将         ║
# ║       fail_rule 插入 index 0。Xray 首匹配规则，fail_rule 覆盖   ║
# ║       所有 relay-in-*，后面的个体规则永远不触发，垃圾堆积。     ║
# ║       v17：Failover 激活时自动清除全部个体 relay 路由规则，      ║
# ║       仅保留 fail_rule(balancer)，配置整洁无冗余。               ║
# ║                                                                  ║
# ║  [J4] BBR 仅在落地机本机启用，中转机未优化                      ║
# ║       CN2 GIA 瓶颈在中国→中转机方向，中转机 BBR 更重要。        ║
# ║       v17：check_and_enable_bbr 完成后，可选通过 SSH 同步在      ║
# ║       中转机启用 BBR（--auto 模式自动执行）。                    ║
# ║                                                                  ║
# ║  [J5] _delete_nodes 删除后重建 Failover 时同样遗留死规则         ║
# ║       v16 _delete_nodes 在重建 balancer 后个体规则仍残留。       ║
# ║       v17：复用 J3 修复逻辑，删除后统一清理个体规则。            ║
# ║                                                                  ║
# ║  [J6] save_result 重复调用 auto_clean_by_link_id                 ║
# ║       v16 read_luodi_info 末尾已调用一次，save_result 头部       ║
# ║       再次调用，打印混淆日志且无实际作用。                       ║
# ║       v17：save_result 移除多余调用。                             ║
# ║                                                                  ║
# ║  [J7] _cleanup_old_data_interactive [2] 全量清除无确认提示       ║
# ║       v16 用户误选 [2] 立即 awk 截断全部对接记录，无法撤回。     ║
# ║       v17：选 [2] 时增加 "确认？[y/N]" 双保险。                  ║
# ║                                                                  ║
# ║  [J8] BBR 版本检测逻辑错误（bbr2 被 bbr 覆盖）                  ║
# ║       v16 先 modprobe tcp_bbr2 → chosen=bbr2，再检测             ║
# ║       kernel>=6 → chosen=bbr，将 bbr2 降级为普通 bbr。          ║
# ║       v17：6.x 内核直接用内置 bbr（即 BBRv3）；5.x 尝试 bbr2    ║
# ║       模块；其余回落 bbr，逻辑正确且有注释说明。                ║
# ║                                                                  ║
# ╠══════════════════════════════════════════════════════════════════╣
# ║  🔧 v16 相较 v15 新增修复                                        ║
# ║                                                                  ║
...
# [J4] 通过 SSH 在中转机也启用 BBR（CN2 GIA 中国→中转方向获益更大）
bbr_on_relay() {
    [[ "$AUTH_TYPE" == "manual" ]] && return 0

    if [[ "$AUTO_MODE" != "true" ]]; then
        local i
        read -rp "同时在中转机启用 BBR？（CN2 GIA 推荐）[Y/n]: " i || true
        [[ "${i,,}" == "n" ]] && return 0
    else
        log_info "自动模式：同步在中转机启用 BBR..."
    fi

    run_relay "
cur=\$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || echo unknown)
if [[ \"\$cur\" == bbr* ]]; then echo \"[OK] 中转机 BBR 已启用: \$cur\"; exit 0; fi
kver=\$(uname -r)
major=\$(echo \"\${kver%%[-]*}\" | cut -d. -f1 || echo 0)
minor=\$(echo \"\${kver%%[-]*}\" | cut -d. -f2 || echo 0)
if (( major > 6 || (major == 6 && minor >= 3) )); then
    chosen=bbr; bbrver='BBRv3'
elif (( major > 5 || (major == 5 && minor >= 13) )); then
    modprobe tcp_bbr2 2>/dev/null && chosen=bbr2 && bbrver='BBR2' || { chosen=bbr; bbrver='BBR1'; }
else
    modprobe tcp_bbr 2>/dev/null || true; chosen=bbr; bbrver='BBR1'
fi
sysctl -w net.core.default_qdisc=fq 2>/dev/null || true
sysctl -w \"net.ipv4.tcp_congestion_control=\${chosen}\" 2>/dev/null || true
grep -q net.core.default_qdisc /etc/sysctl.conf 2>/dev/null || echo 'net.core.default_qdisc = fq' >> /etc/sysctl.conf
grep -q net.ipv4.tcp_congestion_control /etc/sysctl.conf 2>/dev/null || echo \"net.ipv4.tcp_congestion_control = \${chosen}\" >> /etc/sysctl.conf
echo \"[OK] 中转机 \${bbrver} 启用完成（内核 \${kver}）\"
" 2>/dev/null | while read -r l; do log_info "$l"; done || log_warn "中转机 BBR 设置失败，可手动执行"
}
...
p = start_port
# [J2修复] 移除 p>65000 重置逻辑（与计数器相互干扰），直接线性搜索+溢出报错
while p in used or is_port_open(p):
    p += 1
    if p > 65535:
        import sys
        print(f"ERROR: 从 {start_port} 至 65535 均被占用，无法分配端口", file=sys.stderr)
        sys.exit(1)
...
if len(relay_outs) >= 2:
    ...
    # [J3修复] Failover 激活时移除所有个体 relay 路由规则（它们在 fail_rule 之后永不触发）
    cfg["routing"]["rules"] = [r for r in cfg["routing"]["rules"]
                                if r.get("balancerTag") != "relay-balancer"
                                and not (len(r.get("inboundTag",[])) == 1
                                         and r["inboundTag"][0].startswith("relay-in-")
                                         and r.get("outboundTag","").startswith("relay-out-"))]
    cfg["routing"]["rules"].insert(0, fail_rule)
    print(f"[OK] Failover Balancer 已更新（{{len(relay_outs)}} 个落地，个体规则已清理）")
else:
...
generate_substore_json() {
    ...
    # [J1修复] 先在 bash 侧 json.dumps 序列化所有变量，避免含引号/反斜杠时 Python 崩溃
    local _sub_params
    _sub_params=$(python3 -c "
import json
raw_ip = '${RELAY_IP}'.replace('[','').replace(']','')
print(json.dumps({
...
p = json.loads("""${_sub_params}""")

substore_path = p["substore_path"]
link_id       = p["link_id"]
...
proxies = [px for px in proxies if px.get("link_id") != link_id]
...
if len(relay_outs) >= 2:
    relay_ins = [x["tag"] for x in cfg["inbounds"] if x.get("tag","").startswith("relay-in-")]
    fail_rule = {{"type":"field","inboundTag":relay_ins,"balancerTag":"relay-balancer"}}
    # [J5修复] Failover 激活时移除个体 relay 路由规则（dead code）
    cfg["routing"]["rules"] = [r for r in cfg["routing"]["rules"]
                                if r.get("balancerTag") != "relay-balancer"
                                and not (len(r.get("inboundTag",[])) == 1
                                         and r["inboundTag"][0].startswith("relay-in-")
    ...
    print(f"[OK] Balancer 已重建（{{len(relay_outs)}} 个落地，个体规则已清理）")
else:
...
_cleanup_old_data_interactive() {
    ...
    case "$choice" in
        2)
            # [J7修复] 全量清除前要求二次确认，防止误操作
            log_warn "即将清除 ${LOCAL_INFO} 中全部对接记录，此操作不可撤销！"
            local confirm
            read -rp "确认全量清除？输入 yes 确认，其他键取消: " confirm || true
            if [[ "$confirm" != "yes" ]]; then
                log_info "已取消，保留全部记录"
                break
            fi
...
save_result() {
    # [J6修复] 移除重复的 auto_clean_by_link_id 调用
    # （read_luodi_info 末尾已在 LINK_ID 确定后立即清理过一次，此处无需重复）
    {
        echo ""
...
print_result() {
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║  ${GREEN}${BOLD}✓ 对接完成  duijie.sh v17.0${NC}                              ${CYAN}║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "  ${BOLD}流量路径：${NC}"
    ...
    log_sep
    echo -e "${CYAN}v17.0：8项新增修复（J1~J7+J4relay）+ 继承 v16 全部修复（H1~H5）+ 继承 v15 全部修复（G1~G9 + F1~F8 + CB-1~CB-8）${NC}"
    echo ""
}
...
main() {
    ...
    clear
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║       落地机对接脚本  duijie.sh  v17.0                  ║${NC}"
    ...
    [[ "$AUTH_TYPE" != "manual" ]] && bbr_on_relay || true          # [J4] 中转机 BBR 同步
